// Program to demonstrate the memory allocation.
...
int main()
{
    ...
    int *p1 = new int(12);
    int *p2 = p1; // Shallow copy - both point to SAME memory
    ...
    cout << "\n=== Deep Copy (SAFE) ===" << endl;
    int *p3 = new int(25);
    int *p4 = new int(*p3); // Deep copy - NEW memory with same VALUE
    ...
    int *arr2 = new int[3]; // Allocate separate memory
    for (int i = 0; i < 3; i++)
    {
        arr2[i] = arr1[i]; // Copy each element
    }
    delete[] arr1; // arr2 is still safe!
