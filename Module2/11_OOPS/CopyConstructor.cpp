...
public:
    ...
    {
        cout << "Constructor: Allocating memory for " << size << " integers at " << (void *)this << "\n";
        data = new int[size];
    ...
    // Allocates new memory and copies values (not just pointer)
    // This ensures each object has its own independent memory
    DynamicArray(const DynamicArray &source)
    {
        cout << "Copy Constructor: Creating deep copy at " << (void *)this << "\n";
        size = source.size;
        data = new int[size]; // Allocate new memory
        for (int i = 0; i < size; i++)
            data[i] = source.data[i]; // Copy values element by element
    }

    ~DynamicArray()
    {
        cout << "Destructor: Releasing memory at " << (void *)data << "\n";
        delete[] data;
    }
...
// ShallowArray class - demonstrates the danger of default copy constructor
// No custom copy constructor defined, so compiler generates default one
// Default copy constructor only copies pointer value (shallow copy), not the data
class ShallowArray
{
private:
    int *data;
...
public:
    ...
    {
        cout << "Shallow Constructor: Allocating memory at " << (void *)this << "\n";
        data = new int[size];
        for (int i = 0; i < size; i++)
            data[i] = (i + 1) * 100;
    }
    ...
    // This causes both objects to point to same memory location

    ~ShallowArray()
    {
        cout << "Shallow Destructor: Freeing memory at " << (void *)data << "\n";
        delete[] data; // Risk: if two objects share same pointer, double-free error!
    }
...
{
    cout << "\nDemo 1: Deep Copy (Safe)\n";
    cout << "Creating original object (obj1)\n";
    DynamicArray obj1(5);
